...
def admin_employer_detail(request, employer_id):
    ...
    # 🔽 FIX: Calculate total views for this employer's jobs
    ...
    context = {
        ...
        'total_views': total_views,               # 🔽 ADDED this key
...
# ==============================================
# ADDED MISSING EMPLOYER VIEWS
# ==============================================

@login_required
@staff_member_required
def admin_employer_edit(request, employer_id):
    """Edit employer profile"""
    employer = get_object_or_404(EmployerProfile.objects.select_related('user', 'country'), id=employer_id)
    
    if request.method == 'POST':
        form = AdminEmployerForm(request.POST, request.FILES, instance=employer)
        if form.is_valid():
            form.save()
            messages.success(request, f'✅ Employer "{employer.company_name}" updated successfully!')
            return redirect('admin_panel:admin_employer_detail', employer_id=employer.id)
    else:
        form = AdminEmployerForm(instance=employer)
    
    return render(request, 'admin_panel/employers/edit.html', {
        'form': form,
        'employer': employer,
        'is_edit': True,
    })
...
@staff_member_required
@require_POST
def admin_employer_unverify(request, employer_id):
    """Unverify an employer (set is_verified=False)"""
    employer = get_object_or_404(EmployerProfile, id=employer_id)
    employer.is_verified = False
    employer.verification_date = None
    employer.save()
    
    Notification.objects.create(
        user=employer.user,
        title='Employer Unverified',
        message='Your employer account has been unverified. Please contact support.',
        notification_type='employer_unverified'
    )
    
    messages.warning(request, f'⚠️ Employer "{employer.company_name}" unverified.')
    return redirect('admin_panel:admin_employer_detail', employer_id=employer_id)


@login_required
@staff_member_required
def admin_employer_jobs(request, employer_id):
    """List all jobs for a specific employer"""
    employer = get_object_or_404(EmployerProfile, id=employer_id)
    jobs = Job.objects.filter(employer=employer).select_related('country', 'category').order_by('-created_at')
    
    paginator = Paginator(jobs, 20)
    page = request.GET.get('page')
    jobs = paginator.get_page(page)
    
    return render(request, 'admin_panel/employers/jobs.html', {
        'employer': employer,
        'jobs': jobs,
    })