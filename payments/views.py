...
def payment_page(request):
    ...
    context = {
        ...
        'till_number': settings.MPESA_SHORTCODE,  # ✅ Added: Show Till number on payment page
    }
...
def initiate_payment(request):
    ...
    with transaction.atomic():
        payment = Payment.objects.create(
            ...
            # ✅ Added: Track that this is a Till transaction
            mpesa_transaction_type='till',
            till_number=settings.MPESA_SHORTCODE,
        )
...
def process_mpesa_payment(request, payment, phone_number, plan):
    """Process M-Pesa payment - uses Till (Buy Goods)"""
    try:
        mpesa = MpesaService()
        
        # ✅ For Till, use CustomerBuyGoodsOnline (handled in mpesa.py)
        ...
        if response.get('ResponseCode') == '0':
            payment.metadata = {
                ...
                'phone_number': phone_number,
                'response': response,
                'transaction_type': 'till',  # ✅ Added: Track Till usage
                'till_number': settings.MPESA_SHORTCODE,
            }
            payment.save()
...
def payment_confirmation(request, payment_id):
    ...
    context = {
        ...
        'is_till': getattr(payment, 'mpesa_transaction_type', None) == 'till',  # ✅ Added
        'till_number': getattr(payment, 'till_number', settings.MPESA_SHORTCODE),  # ✅ Added
    }
...
def download_receipt(request, payment_id):
    ...
    try:
        ...
        # ✅ Added: Show Till number if it's a Till transaction
        if getattr(payment, 'mpesa_transaction_type', None) == 'till':
            payment_details.append(['Transaction Type:', 'M-Pesa Till (Buy Goods)'])
            payment_details.append(['Till Number:', getattr(payment, 'till_number', 'N/A')])
        ...
        # ✅ Added: Show Till number if it's a Till transaction
        if getattr(payment, 'mpesa_transaction_type', None) == 'till':
            content += f"\nTransaction Type: M-Pesa Till (Buy Goods)"
            content += f"\nTill Number: {getattr(payment, 'till_number', 'N/A')}"
        if payment.mpesa_receipt_number:
...
def download_invoice(request, payment_id):
    ...
    try:
        ...
        # ✅ Added: Show Till number if it's a Till transaction
        if getattr(payment, 'mpesa_transaction_type', None) == 'till':
            amount_data.append(['Transaction Type:', 'M-Pesa Till (Buy Goods)'])
            amount_data.append(['Till Number:', getattr(payment, 'till_number', 'N/A')])
...
def admin_payments(request):
    ...
    # ✅ Added: Till-specific stats
    till_payments = Payment.objects.filter(mpesa_transaction_type='till').count()
    till_revenue = Payment.objects.filter(mpesa_transaction_type='till', status='completed').aggregate(
        total=Sum('amount')
    )['total'] or 0
...
def payment_reports(request):
    ...
    # ✅ Added: Till vs Paybill breakdown
    till_data = payments.filter(
        mpesa_transaction_type='till',
        status='completed'
    ).aggregate(
        total=Sum('amount'),
    ...
    context = {
        ...
        # ✅ Added: Till stats
        'till_total': till_data['total'] or 0,
        'till_count': till_data['count'] or 0,
    }
...
def export_payments(request):
    ...
    writer.writerow([
        ...
        'Payment Method', 'Transaction Type', 'Till Number', 'Status', 'Date', 'Receipt Number'  # ✅ Added columns
    ])
    ...
    for payment in payments:
        writer.writerow([
            ...
            payment.get_mpesa_transaction_type_display() if hasattr(payment, 'get_mpesa_transaction_type_display') else '',
            getattr(payment, 'till_number', ''),
...
def revenue_report(request):
    ...
    # ✅ Added: Revenue by transaction type (Till vs Paybill)
    revenue_by_type = payments.values('mpesa_transaction_type').annotate(
        total=Sum('amount'),
        count=Count('id')
    ).order_by('-total')
    ...
    context = {
        ...
        'revenue_by_type': revenue_by_type,  # ✅ Added
    }
    ...
    try:
        ...
        if response.get('ResponseCode') == '0':
            ...
            return JsonResponse({
                ...
                'transaction_reference': transaction_ref,
                'transaction_type': 'till',  # ✅ Added
                'till_number': settings.MPESA_SHORTCODE,  # ✅ Added
            })
        else:
            payment.status = 'failed'
            payment.save()
...
def api_payment_status(request, payment_id):
    ...
    return JsonResponse({
        ...
        'receipt_number': payment.mpesa_receipt_number,
        'transaction_type': getattr(payment, 'mpesa_transaction_type', None),  # ✅ Added
        'till_number': getattr(payment, 'till_number', None),  # ✅ Added
    })
...
def test_mpesa(request):
    ...
    if settings.DEBUG:
        ...
        return render(request, 'payments/test_mpesa.html', context)
    return redirect('payments:payment_page')
